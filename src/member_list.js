const member_list = {
  members: {
    ptarsoly: {
      id: "ptarsoly",
      img_src: "./public/ptarsoly.png",
      name: "Peter W. Tarsoly",
      major: "Computer Engineering",
      number: 1,
      class: "2021",
      origin: "Huntington, NY",
      catchphrase: "",
    },
    gjacobellis: {
    id: "gjacobellis",
      img_src: "./public/gjacobellis.png",
      name: "Garrett A. Jacobellis",
      major: "Mechanical Engineering",
      number: 2,
      class: "2020",
      origin: "Huntington, NY",
      catchphrase: "",
    },
    athierer: {
    id: "athierer",
      img_src: "",
      name: "Alex Thierer",
      major: "Civil Engineering",
      number: "",
      class: "2020",
      origin: "",
      catchphrase: "",
    },
    dmccook: {
    id: "dmccook",
      img_src: "./public/dmccook.png",
      name: "Dylan L. McCook",
      major: "Aerospace Engineering",
      number: 3,
      class: "2022",
      origin: "Liveoak, FL",
      catchphrase: "",
    },
    anoori: {
    id: "anoori",
      img_src: "./public/anoori.png",
      name: "Amanda Noori",
      major: "Aerospace Engineering",
      number: 4,
      class: "2019",
      origin: "Atlanta, GA",
      catchphrase: "",
    },
    dsouthwell: {
    id: "dsouthwell",
      img_src: "./public/dsouthwell.png",
      name: "Donovan Southwell",
      major: "Aerospace Engineering",
      number: 5,
      class: "2020",
      origin: "Indiatlantic, FL",
      catchphrase: "",
    },
    msyed: {
    id: "msyed",
      img_src: "./public/msyed.png",
      name: "Muntaser Syed",
      major: "Computer Engineering",
      number: 6,
      class: "2022",
      origin: "Abu Dhabi, UAE",
      catchphrase: "",
    },
    athierer: {
    id: "athierer",
      img_src: "./public/athierer.png",
      name: "Alexander B. Thierer",
      major: "Civil Engineering",
      number: 7,
      class: "2019",
      origin: "High Point, NC",
      catchphrase: "",
    },
    athornton: {
    id: "athornton",
      img_src: "./public/athornton.png",
      name: "Alana Thornton",
      major: "Mechanical Engineering",
      number: 8,
      class: "2019",
      origin: "Fort Washington, MD",
      catchphrase: "",
    },
    rwhitman: {
    id: "rwhitman",
      img_src: "./public/rwhitman.png",
      name: "Robert Whitman",
      major: "Computer Science",
      number: 9,
      class: "2021",
      origin: "Palm Bay, FL",
      catchphrase: "",
    },
    fswarr: {
    id: "fswarr",
      img_src: "./public/fswarr.png",
      name: "Fiona E. Swarr",
      major: "Mechanical Engineering",
      number: 10,
      class: "2022",
      origin: "Melbourne, FL",
      catchphrase: "",
    },
    sedwards: {
    id: "sedwards",
      img_src: "./public/sedwards.png",
      name: "Sarah Edwards",
      major: "Aerospace Engineering",
      number: 11,
      class: "202",
      origin: "Brentwood, TN",
      catchphrase: "",
    },
    crush: {
    id: "crush",
      img_src: "./public/crush.png",
      name: "Cassandra Rush",
      major: "Aerospace Engineering",
      number: 12,
      class: "2022",
      origin: "Pittsburgh, PA",
      catchphrase: "",
    },
    ahopkins: {
    id: "ahopkins",
      img_src: "./public/ahopkins.png",
      name: "Alexis Hopkins",
      major: "Biomedical Engineering",
      number: 13,
      class: "2021",
      origin: "",
      catchphrase: "",
    },
    mtapaszto: {
    id: "mtapaszto",
      img_src: "./public/mtapaszto.png",
      name: "Marcell Tapaszto",
      major: "Computer Engineering",
      number: 15,
      class: "2022",
      origin: "Kecskemet, Hungary",
      catchphrase: "",
    },
    measton: {
    id: "measton",
      img_src: "./public/measton.png",
      name: "Miranda Easton",
      major: "Aerospace Engineering",
      number: 16,
      class: "2023",
      origin: "Tallahassee, FL",
      catchphrase: "",
    },
    rgriffin: {
    id: "rgriffin",
      img_src: "./public/rgriffin.png",
      name: "Ryley P. Griffin",
      major: "Chemical Engineering",
      number: 17,
      class: "2022",
      origin: "Morgantown, WV",
      catchphrase: "",
    },
    lblackwell: {
    id: "lblackwell",
      img_src: "./public/lblackwell.png",
      name: "Landon C. Blackwell-Frank",
      major: "Mechanical Engineering",
      number: 18,
      class: "2024",
      origin: "The Woodlands, TX",
      catchphrase: "",
    },
    jhing: {
    id: "jhing",
      img_src: "./public/jhing.png",
      name: "Justing Hing",
      major: "Aerospace Engineering",
      number: 19,
      class: "2022",
      origin: "fort Lauterdale, FL",
      catchphrase: "",
    },
    mhoadley: {
    id: "mhoadley",
      img_src: "./public/mhoadley.png",
      name: "Michael Hoadley",
      number: 20,
      major: "Mechanical Engineering",
      class: "2022",
      origin: "Branford, CT",
      catchphrase: "Rockets and friends. Who needs more?",
    },
    ileon: {
    id: "ileon",
      img_src: "./public/ileon.png",
      name: "Ian Leon",
      major: "Aerospace Engineering",
      number: 21,
      class: "2022",
      origin: "San Jose, Costa Rica",
    },
    csisko: {
      id: "csisko",
      img_src: "./public/csisko.png",
      name: "Caroline Sisko",
      major: "Aerospace Engineering",
      number: 22,
      class: "2022",
      origin: "Inverness, FL",
      catchphrase: "",
    },
    rhutchinson: {
    id: "rhutchinson",
      img_src: "./public/rhutchinson.png",
      name: "Ryan Hutchinson",
      major: "Aerospace Engineering",
      number: 23,
      class: "2022",
      origin: "Cocoa, FL",
      catchphrase: "",
    },
    dnieves: {
    id: "dnieves",
      img_src: "./public/dnieves.png",
      name: "David Nieves Acarón",
      major: "Computer Engineering",
      number: 24,
      class: "2022",
      origin: "San Juan, Puerto Rico",
      catchphrase: "",
    },
    grivasleal: {
    id: "grivasleal",
      img_src: "./public/grivasleal.png",
      name: "Gerardo Rivas Leal",
      major: "Computer Engineering",
      number: 25,
      class: "2022",
      origin: "Labelle, FL",
      catchphrase: "",
    },
    atisaranni: {
    id: "atisaranni",
      img_src: "./public/atisaranni.png",
      name: "Ashley L. Tisaranni",
      major: "Aerospace Engineering",
      number: 26,
      class: "2023",
      origin: "Germantown, MD",
      catchphrase: "",
    },
    kdecker: {
    id: "kdecker",
      img_src: "./public/kdecker.png",
      name: "Kayla N. Decker",
      major: "Aerospace Engineering",
      number: 27,
      class: "2022",
      origin: "Jacksonville, FL",
      catchphrase: "",
    },
    kforero: {
    id: "kforero",
      img_src: "./public/kforero.png",
      name: "Kathleen Forero",
      major: "Chemical Engineering",
      number: 28,
      class: "2023",
      origin: "",
      catchphrase: "",
      include: false,
    },
    amichalczyk: {
    id: "amichalczyk",
      img_src: "./public/kforero.png",
      name: "Alaina M. Michalczyk",
      major: "Mechnical Engineering",
      number: 29,
      class: "2023",
      origin: "Chayna",
      catchphrase: "",
      include: false,
    },
    asullivan: {
    id: "asullivan",
      img_src: "./public/asullivan.png",
      name: "Adli R. Sullivan",
      major: "Chemical Engineering",
      number: 30,
      class: "2023",
      origin: "",
      catchphrase: "",
    },
    swaddell: {
    id: "swaddell",
      img_src: "./public/swaddell.png",
      name: "Shelton Waddell",
      major: "Aerospace Engineering",
      number: 31,
      class: "2023",
      origin: "Melbourne, FL",
      catchphrase: "",
    },
    rblomquist: {
    id: "rblomquist",
      img_src: "./public/rblomquist.png",
      name: "Regan Blomquist",
      major: "Biomedical Engineering",
      number: 32,
      class: "2025",
      origin: "",
      catchphrase: "",
    },
    gmarrero: {
    id: "gmarrero",
      img_src: "./public/gmarrero.png",
      name: "Jay Marrero",
      major: "Aerospace Engineering",
      number: 33,
      class: "2023",
      origin: "Flushing, NY",
      catchphrase: "",
    },
    kmobley: {
    id: "kmobley",
      img_src: "./public/kmobley.png",
      name: "Ken Mobley",
      major: "Aerospace Engineering",
      number: 34,
      class: "2025",
      origin: "",
      catchphrase: "",
    },
    tpaige: {
    id: "tpaige",
      img_src: "./public/tpaige.png",
      name: "Trent Paige",
      major: "Aerospace Engineering",
      number: 35,
      class: "2025",
      origin: "",
      catchphrase: "",
    },
    cswords: {
    id: "cswords",
      img_src: "./public/cswords.png",
      name: "Carlie Swords",
      major: "Aerospace Engineering",
      number: 36,
      class: "2025",
      origin: "",
      catchphrase: "",
    },
    dbarnes: {
    id: "dbarnes",
      img_src: "./public/dbarnes.png",
      name: "Dylan Barnes",
      major: "Aerospace Engineering",
      number: 37,
      class: "2022",
      origin: "",
      catchphrase: "",
    },
    rhalcyon: {
    id: "rhalcyon",
      img_src: "./public/rhalcyon.png",
      name: "Rosemary Halcyon-Schmitt",
      major: "Chemical Engineering",
      number: 38,
      class: "",
      origin: "",
      catchphrase: "",
    },

    jjohnson: {
    id: "jjohnson",
      img_src: "./public/jjohnson.png",
      name: "Jordan Johnson",
      major: "Civil Engineering",
      number: 39,
      class: "2025",
      origin: "",
      catchphrase: "",
    },

    ajunkin: {
    id: "ajunkin",
      img_src: "./public/ajunkin.png",
      name: "Andrew Junkin",
      major: "Biomedical Engineering",
      number: 40,
      class: "2023",
      origin: "",
      catchphrase: "",
    },
    mlanahan: {
    id: "mlanahan",
      img_src: "./public/mlanahan.png",
      name: "Madison Lanahan",
      major: "Computer Science ",
      number: 41,
      class: "2026",
      origin: "",
      catchphrase: "",
    },
    dmcdaniels: {
      id: "dmcdaniels",
      img_src: "./public/dmcdaniels.png",
      name: "Devlin McDaniels",
      major: "Aerospace Engineering",
      number: 42,
      class: "2022",
      origin: "",
      catchphrase: "",
    },
    vshirley: {
    id: "vshirley",
      img_src: "./public/vshirley.png",
      name: "Val Shirley",
      major: "Aerospace Engineering",
      number: 43,
      class: "2023",
      origin: "",
      catchphrase: "",
    },
    kweaver: {
    id: "kweaver",
      img_src: "./public/kweaver.png",
      name: "Keelin Weaver",
      major: "Aerospace Engineering",
      number: 44,
      class: "2025",
      origin: "",
      catchphrase: "",
    },
    czinninger: {
    id: "czinninger",
      img_src: "./public/czinninger.png",
      name: "Charlotte Zinninger",
      major: "Aerospace Engineering",
      number: 45,
      class: "2025",
      origin: "Syracyse, NY",
      catchphrase: "",
    },
    oloris: {
    id: "oloris",
      img_src: "./public/oloris.png",
      name: "Olivia Loris",
      major: "Software Engineering",
      number: 46,
      class: "2026",
      origin: "",
      catchphrase: "",
    },
    gspitaletto: {
    id: "gspitaletto",
      img_src: "./public/gspitaletto.png",
      name: "Genevieve Spitaletto",
      major: "Civil Engineering",
      number: 47,
      class: "",
      origin: "",
      catchphrase: "",
    },
    astringer: {
    id: "astringer",
      img_src: "./public/astringer.png",
      name: "Ashton Stringer",
      major: "Mechanical Engineering",
      number: 48,
      class: "2025",
      origin: "",
      catchphrase: "",
    },
    zhardin: {
    id: "zhardin",
      img_src: "./public/zhardin.png",
      name: "Zachary Hardin",
      major: "Mechanical Engineering",
      number: 49,
      class: "2026",
      origin: "Rock Island, Illinois",
      catchphrase: "",
    },
    amoore: {
    id: "amoore",
      img_src: "./public/amoore.png",
      name: "Archer Moore",
      major: "Chemical Engineering",
      number: 50,
      class: "2024",
      origin: "",
      catchphrase: "",
    },
    gmorales: {
      id: "gmorales",
      img_src: "./public/gmorales.png",
      name: "Gabriel Morales",
      major: "Software Engineering",
      number: 51,
      class: "2026",
      origin: "North Carolina",
      catchphrase: "",
    },
  },
  regents: {
    previous: {
      "2018-2019": ["crush"], // Peter
      "2020-2021": ["fswarr"], // Fiona
      "2021-2022": ["measton", "ileon"], // Ran, Ian
    },
    current: "gmarrero", // Jay
  },
  linkDataArray: [
    { source: "dsouthwell", target: "mtapaszto" },
    { source: "mtapaszto", target: "mhoadley" },
    { source: "mhoadley", target: "dnieves" },
    { source: "mhoadley", target: "rblomquist" },
    { source: "dnieves", target: "czinninger" },
    { source: "dnieves", target: "zhardin" },
    { source: "czinninger", target: "gmorales" },
    {source:"athierer", target: "sedwards"},
    {source:"sedwards", target: "jhing" },
    {source: "jhing", target: "grivasleal" },
    {source: "jhing", target: "gmarrero" }
  ],
};

export default member_list;
